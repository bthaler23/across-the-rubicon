...
namespace Game.Grid
{
	public class PlaneXZHexGridDraw : MonoBehaviour
	{
		[SerializeField]
		...
		private List<Vector2> GetGridPositions()
		{
			...
			else
			{
				...
				{
					position.Add(HexGridManager.Instance.GridIndexToWordPosition(p));
				}
				return position;
			}
		}
		...
		private List<Vector2> GetAreaDrawPositions()
		{
			...
			Vector2 gridCenter = HexGridManager.Instance.GridIndexToWordPosition(gridCenterIndex);
			...
			for (int x = gridCenterIndex.x - gridWidthHalf; x < gridCenterIndex.x + gridWidthHalf; x++)
			{
				for (int y = gridCenterIndex.y - gridHeightHalf; y < gridCenterIndex.y + gridHeightHalf; y++)
				{
					...
					Vector2 worldPos = HexGridManager.Instance.GridIndexToWordPosition(position);
					bool enableDraw =
						gridDrawRadius == 0
